...
**Controller:** App\Http\Controllers\Nafsul\PekerjaanController
**Base URL:** /api/nafsul/pekerjaan

Master pekerjaan. Tabel & kolomnya berbahasa Inggris (`occupations.name`),
...
form anggota merujuknya lewat `pekerjaan_id`, bukan kode.

---

## Keunikan `nama` (mengabaikan huruf besar-kecil)

`"Guru"`, `"guru"`, dan `"GURU"` terhitung **satu** pekerjaan yang sama. Dijaga di
tiga lapis:
...
| Index `occupations_name_unique` | Jaring pengaman untuk permintaan yang berjalan bersamaan |
...
**Method:** GET · **Endpoint:** /api/nafsul/pekerjaan · **Auth:** Bearer Token (wajib)
...
**Method:** POST · **Endpoint:** /api/nafsul/pekerjaan · **Auth:** Bearer Token (wajib)
...
```json
{
  ...
  "errors": { "nama": ["Pekerjaan \"guru\" sudah ada."] }
}
```
...
**Endpoint:** /api/nafsul/pekerjaan/{occupation} · **Auth:** Bearer Token (wajib)
...
**Method:** POST · **Endpoint:** /api/nafsul/pekerjaan/import · **Auth:** Bearer Token (wajib)
...
```json
{
  ...
  "gagal": 1,
  "hasil": [
    { "baris": 2, "status": "ok", "id": 28, "nama": "Guru" },
    { "baris": 3, "status": "ok", "id": 29, "nama": "Petani" },
    {
      "baris": 4,
      "status": "gagal",
      "nama": "GURU",
      "pesan": "Pekerjaan \"GURU\" sudah ada (tercatat sebagai \"Guru\")."
    }
  ]
}
```
...
Modal impornya: `components/nafsul/ImportPekerjaanModal.tsx` (fe-care-pulse).
